(function (global, factory) {
  ...
  (function ($) {
    ...
    var DEFAULTS = {
      ...
      paginationDoublePreText: '&lsaquo;&lsaquo;',
      paginationNextText: '&rsaquo;',
      paginationDoubleNextText: '&rsaquo;&rsaquo;',
    ...
    var BootstrapTable = function () {
      ...
      _createClass(BootstrapTable, [{
        ...
        value: function initContainer() {
          var topPagination = ['top', 'both'].indexOf(this.options.paginationVAlign) !== -1 ? '<div class="fixed-table-pagination pagination-top clearfix"></div>' : '';
          var bottomPagination = ['bottom', 'both'].indexOf(this.options.paginationVAlign) !== -1 ? '<div class="fixed-table-pagination pagination-bottom"></div>' : '';
...
//          var order = this.options.sortOrder === 'desc' ? -1 : 1;
          var order = this.options.sortOrder === 'asc' ? -1 : 1;
...
//              this.options.sortOrder = $this.data('order') === 'asc' ? 'desc' : 'asc';
            	this.options.sortOrder = $this.data('order') === 'desc' ? 'asc' : 'desc';
            } else {
        ...
        value: function initPagination() {
          ...
          html.push('<div class="' + this.constants.classes.pull + '-' + o.paginationDetailHAlign + ' pagination-detail row">\n        <span class="col-6 pagination-info" style="margin:0;">\n        ' + paginationInfo + '\n        </span>');

          if (!o.onlyInfoPagination) {
            html.push('<span class="text-right col-6 page-list">');
            ...
            html.push('<div class="' + this.constants.classes.pull + '-' + o.paginationHAlign + ' pagination">', Utils.sprintf(this.constants.html.pagination[0], Utils.sprintf(' pagination-%s', o.iconSize)), Utils.sprintf(this.constants.html.paginationItem, ' page-double-pre', o.paginationDoublePreText), Utils.sprintf(this.constants.html.paginationItem, ' page-pre', o.paginationPreText));
            if (this.totalPages < o.paginationSuccessivelySize) {
              from = 1;
              to = this.totalPages;
            } else {
              //from = o.pageNumber - o.pagination~PagesBySide;
              //to = from + o.paginationSuccessivelySize - 1;
              from = o.pageNumber - ((o.pageNumber - 1) % o.paginationSuccessivelySize);
              to = from + (o.paginationSuccessivelySize - 1);
            }
            ...
            */
            ...
            html.push(Utils.sprintf(this.constants.html.paginationItem, ' page-double-next', o.paginationDoubleNextText));
          ...
          if (!o.onlyInfoPagination) {
            ...
            $dpre = this.$pagination.find('.page-double-pre');
            $dnext = this.$pagination.find('.page-double-next');
            $number = this.$pagination.find('.page-item').not('.page-next, .page-pre, .page-double-next, .page-double-pre');
            ...
            if (!o.paginationLoop) {
              /*
               * 2019 - 10 - 07 
               * 2이상의  page 에서  '<' 버튼 클릭시 에러 
               * 음수 페이지를 1로 수정.
               * when you change to page of minus number, 
               * change page number from minus to one
               */
              if(o.pageNumber < 1){
                  o.pageNumber = 1;
              }
//              if (o.pageNumber === 1) {
              if (o.pageNumber <= o.paginationSuccessivelySize) {
                  $pre.addClass('disabled');
                  $dpre.addClass('disabled');
              }
            ...
            $dpre.off('click').on('click', function (e) {
              return _this6.onPageDoublePre(e);
            });
            $dnext.off('click').on('click', function (e) {
              return _this6.onPageDoubleNext(e);
            });
        ...
        value: function onPagePre(event) {
          event.preventDefault();
          var temp = (Math.floor((this.options.pageNumber - 1)/ this.options.paginationSuccessivelySize) - 1) * this.options.paginationSuccessivelySize + 1;
          if (this.options.pageNumber < 1) {
            this.options.pageNumber = 1;
          } else {
            //this.options.pageNumber--;
              this.options.pageNumber = temp;
          }
          this.updatePagination(event);
          return false;
        }
        ...
        value: function onPageNext(event) {
          event.preventDefault();
          var temp = (Math.floor((this.options.pageNumber - 1)/ this.options.paginationSuccessivelySize) + 1) * this.options.paginationSuccessivelySize + 1;
          if (temp > this.options.totalPages) {
          ...
          key: 'onPageDoublePre',
          value: function onPageDoublePre(event) {
            event.preventDefault();
            this.options.pageNumber = 1;
            this.updatePagination(event);
            return false;
          ...
          key: 'onPageDoubleNext',
          value: function onPageDoubleNext(event) {
            event.preventDefault();
            this.options.pageNumber = this.options.totalPages;
            this.updatePagination(event);
            return false;
        ...
        value: function onPageNumber(event) {
          ...
          if (this.options.pageNumber === target) {
            return;
          }
          this.options.pageNumber = target;
          this.updatePagination(event);
          return false;
        }
      }, {
...
//            }
//            else {
//            	this.$tableBorder && this.$tableBorder.css('height', height - tableHeight - 1 + 'px');
//            }
          this.$tableBorder && this.$tableBorder.css('height', height - tableHeight - padding - 1 + 'px');
          }
