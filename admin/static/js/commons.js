$(function() {
	...
	// goto list
	$('.btn-list').off('click').on('click', function() {
...
// });
	$summernote.summernote({
		lang: 'ko-KR',
		height: 200,
		toolbar: [['fontstyle', ['bold', 'italic', 'underline', 'strikethrough', 'clear']], ['fontsize'], ['color'], ['para', ['ul', 'ol', 'paragraph']],
			['height'], ['insert', ['picture', 'link', 'table', 'hr']], ['fullscreen'], ['codeview']],
...
$.fn.defaultDatepicker = function(option = {}) {
	...
	var datepicker = $base.datepicker(Object.assign({
		format : 'yyyy-mm-dd',
		autoclose : true
	}, option));
	...
	// 기간 입력 인 경우 선택 범위 제한
	$base.closest('.input-group').find('input').each(function() {
		var name = this.name;
...
function setPopOverImg(position){
	...
	$('[data-toggle="popover-hover"]').popover({
		...
		placement: position,
		sanitize: false,
		content: function(){return '<img src="'+$(this).data('img')+'" style="max-width: 100%;" />';}
	});
}
